...
export const duplicateNode: CommandHandler = async (params) => {
  const { nodeId, newName, newParentId, offset } = params as {
    nodeId: string;
    newName?: string;
    newParentId?: string;
  ...
  cloned.name = newName || `${node.name} Copy`;
...
export const ungroupNodes: CommandHandler = async (params) => {
  ...
  for (const id of nodeIds) {
    ...
    for (const child of children) {
      ...
      (parent as ChildrenMixin).insertChild(frameIndex, child);
    }
...
export const swapComponent: CommandHandler = async (params) => {
  const { instanceId, newComponentId } = params as {
    instanceId: string;
  ...
  const newComponentNode = figma.getNodeById(newComponentId);
  if (!newComponentNode || newComponentNode.type !== 'COMPONENT') {
    throw new Error(`Component not found: ${newComponentId}`);
  }

  const newComponent = newComponentNode as ComponentNode;

  // swapComponent 是 Figma Plugin API 的方法
